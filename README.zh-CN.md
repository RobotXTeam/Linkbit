...
这个场景的核心是：朋友的电脑先作为一台新设备加入 Linkbit 网络，然后你在自己电脑上开一个本地端口，把流量通过 Linkbit 云中继转到朋友电脑的远程桌面端口。
...
2. 在页面顶部输入 `Admin API Key`，点击 `连接`。

`Admin API Key` 是管理员密钥，只给你自己使用，不要发给朋友。当前测试部署中，它保存在本机：
...
点击 `生成` 后，会出现一次性邀请码。邀请码只显示一次，只用于朋友电脑第一次加入网络。
...
5. 朋友安装 Linkbit，并以管理员身份启动 Agent。朋友端填写：
...
设备名称建议使用简单英文，比如 `friend-pc`、`office-pc`、`laowang-pc`。后面你连接时就用这个名字。

6. 朋友电脑需要开启一个可被连接的远程控制服务。常见端口如下：
...
如果朋友电脑是 Windows Pro/Enterprise，可以启用系统自带远程桌面，端口就是 `3389`。如果是 Windows Home，通常没有内置 RDP 服务，可以安装 NoMachine、VNC 或 RustDesk。

7. 朋友电脑出现在管理后台后，你在自己电脑的 Linkbit 桌面端打开 `SSH/RDP 中转`。如果朋友电脑使用 Windows RDP，填写：
...
这里的 `3389` 是朋友电脑上的目标服务端口；`13389` 是你自己电脑上的本地端口，可以换成其他未占用端口。
...
9. 如果你连接的不是 RDP，而是其他服务，把远端目标端口换掉即可：

```text
远程 SSH：friend-pc:22，本地监听可用 127.0.0.1:10022
远程 VNC：friend-pc:5900，本地监听可用 127.0.0.1:15900
远程 NoMachine：friend-pc:4000，本地监听可用 127.0.0.1:14000
```

注意：Linkbit 负责把你的电脑和朋友电脑通过云服务器中转连起来；朋友电脑本身还需要开启远程桌面或远程控制服务。如果朋友电脑没有这些服务，只安装 Linkbit 还不能直接看到屏幕。
