...
In this scenario, your friend's computer first joins your Linkbit network as a new device. Then you open a local port on your computer and forward it through the Linkbit cloud relay to the remote desktop port on your friend's computer.
...
2. Paste the `Admin API Key` at the top of the page, then click `Connect`.

The `Admin API Key` is only for you as the administrator. Do not send it to your friend. For the current test deployment, it is stored locally:

```bash
cat .tools/remote-bootstrap-key
```

3. In `Device Invitation`, keep the defaults:
...
Click `Generate`. The console shows a one-time enrollment token. It is shown once and is only used for the friend's first registration.

4. Send these three things to your friend:

```text
Linkbit client installer
Controller URL: http://120.79.155.227
Enrollment token generated by the web console
```

5. Your friend installs Linkbit and starts the Agent with administrator/root permission. On your friend's computer, enter:
...
Use a simple English device name such as `friend-pc`, `office-pc`, or `laowang-pc`. You will use this name later as the forwarding target.

6. Your friend's computer must run a service that can accept remote-control connections. Common ports are:
...
NoMachine: 4000
SSH: 22
Web service: 80 or 8080
```

If your friend's computer is Windows Pro or Enterprise, enable built-in Remote Desktop; its default port is `3389`. If it is Windows Home, built-in RDP is usually unavailable, so install NoMachine, VNC, or RustDesk instead.

7. After `friend-pc` appears in the console, open `SSH/RDP Forwarding` in your Linkbit desktop client. For Windows RDP, enter:
...
Here `3389` is the service port on your friend's computer. `13389` is the local port on your own computer; you can change it to another unused local port.

8. Open your remote desktop client and connect to:

```text
127.0.0.1:13389
```

9. If you are forwarding something other than RDP, change the remote target port:

```text
Remote SSH: friend-pc:22, local listen can be 127.0.0.1:10022
Remote VNC: friend-pc:5900, local listen can be 127.0.0.1:15900
Remote NoMachine: friend-pc:4000, local listen can be 127.0.0.1:14000
```

Linkbit provides the private cloud relay path between your computer and your friend's computer. The friend's computer must still run a remote desktop or remote-control service. If no such service is running on your friend's computer, installing Linkbit alone is not enough to see the screen.
